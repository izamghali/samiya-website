...
function carouselLoading(bar, layer, fakeLayer, logo, line) {
...
function Carousel(props) {
    ...
    let loadingBar = useRef(null)
    let fakeLayer = useRef(null)
    ...
    useEffect(() => {
        ...
        img.onload = () => {
            setImageLoaded(true) 
            carouselLoading(loadingBar, layer, fakeLayer, logo, line)
        }
    ...
    }, [carouselDisplayed, index])
    
    return (
        <section className="overflow-hidden relative">

            <div ref={el => loadingBar = el} className="bg-white opacity-30 absolute top-0 left-0 lg:h-[0.5rem] w-[0rem] z-10" ></div>
            <div ref={el => fakeLayer = el} className="hidden lg:block bg-stone-950 absolute top-0 left-0 w-full h-full"></div>
            {/* bg-gradient-to-b from-[rgba(0,0,0,0)] to-stone-950  */}
            <div ref={el => layer = el} className="bg-gradient-to-b from-[rgba(0,0,0,0)] to-stone-950 absolute top-0 left-0 w-full h-full"></div>

            <div className="
                sm:bottom-16 sm:scale-[60%]
                md:bottom-20 md:scale-75
                lg:bottom-36 lg:scale-90
                xl:bottom-40 xl:scale-100
                scale-50 duration-300
                absolute bottom-12 right-0 left-0 
                mx-auto w-64">
                <svg ref={el => logo = el} className="translate-y-0 opacity-100 lg:translate-y-[20%] lg:opacity-0 fill-white w-full h-full" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 514 138" version="1.1">
                ...
                <p ref={el => line = el} className="translate-y-0 opacity-100 lg:translate-y-[20%] lg:opacity-0 sm:block hidden text-stone-300 font-light text-center mt-4 italic tracking-widest">Capture the perfect one!</p>
            </div>
